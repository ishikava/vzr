...
class IntegerHelper
{
    ...
    {
        $charsetVariants = StringHelper::getLength($charsetList);
	...
	public static function getBytes($length)
    {
        ...
        if (static::isOpensslAvailable()) {
            ...
            if ($bytes && StringHelper::getLength($bytes) >= $length) {
                if ($strong)
                    return StringHelper::getSubstring($bytes, 0, $length);
                else
                    $backup = $bytes;
            }
        }
        ...
        if ($backup && StringHelper::getLength($backup) >= $length) {
            return StringHelper::getSubstring($backup, 0, $length);
        }

        $bytes = '';
        while (StringHelper::getLength($bytes) < $length) {
	...
	protected static function getPseudoRandomBlock()
    {
        ...
        if (static::isOpensslAvailable()) {
            ...
            if ($bytes && StringHelper::getLength($bytes) >= static::RANDOM_BLOCK_LENGTH) {
                return StringHelper::getSubstring($bytes, 0, static::RANDOM_BLOCK_LENGTH);
            }
        }
